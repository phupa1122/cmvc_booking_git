                                ...
                                <tr class="text-center">
                                    ...
                                    <td>
                                        @foreach ($room->equipment as $item)
                                            {{ $item->equipment->name }}: {{ $item->quantity }} <br>
                                        @endforeach
                                    </td>
                                    @if (auth()->user()->is_admin == 1)
                                        <td class="text-center">
                                            <a href="{{ route('meeting-rooms.edit', $room->id) }}"
                                                class="btn btn-warning btn-sm me-2">
                                                <i class="fas fa-edit me-1"></i>แก้ไข
                                            </a>
                                            <form action="{{ route('meeting-rooms.destroy', $room->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm"
                                                    onclick="return confirm('คุณแน่ใจว่าต้องการลบห้องประชุมนี้?')">
    ...
    <script>
        $(document).ready(function() {
            $('#meetingRoomTable').DataTable({
                ...
                columnDefs: [{
                    orderable: false,
                    targets: {{ auth()->user()->is_admin == 1 ? 6 : '' }}
                }]
            });
        });
    </script>
